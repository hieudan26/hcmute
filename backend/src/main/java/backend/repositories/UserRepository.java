...
public interface UserRepository extends PagingAndSortingRepository<Users, String>, JpaSpecificationExecutor<Users> {
    ...
    @Query("select friends from Friends friends  inner join Users user  on user = friends.owner where user.id = :userId and friends.status <> 'no_friend' and user.isDisable = false order by friends.time desc ")
    Page<Friends> getFriends(String userId, Pageable pageable);
    @Query("select friends from Friends friends  inner join Users user  on user = friends.owner where user.id = :userId and friends.status <> 'no_friend' and user.isDisable = false and (friends.friend.firstName LIKE CONCAT('%', :key, '%') OR friends.friend.lastName LIKE CONCAT('%', :key, '%')) order by friends.time desc ")
    Page<Friends> getFriendsWithSearch(String userId, String key, Pageable pageable);

    @Query("select friends from Friends friends inner join  Users user on user = friends.owner where user.id = :userId and friends.status = :status and friends.status <> 'no_friend' and user.isDisable = false order by friends.time desc ")
    Page<Friends> getFriendsByStatus(String userId, String status, Pageable pageable);
    @Query("select friends from Friends friends inner join  Users user on user = friends.owner where user.id = :userId and friends.status = :status and friends.status <> 'no_friend' and user.isDisable = false and (friends.friend.firstName LIKE CONCAT('%', :key, '%') OR friends.friend.lastName LIKE CONCAT('%',:key, '%'))  order by friends.time desc ")
