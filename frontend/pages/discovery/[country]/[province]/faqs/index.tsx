...
const ProvinceFaqs: NextPage = (props: IProvinceFaqsProps) => {
  ...
  }, [data, dataCountry.data?.data.hashTags]);
        ...
        <Box mb='4'>
          <Breadcrumb spacing='8px' separator={<ChevronRightIcon color='gray.500' />}>
            ...
            <BreadcrumbItem isCurrentPage>
              <Link href={`/discovery/${country}/${province}/faqs`}>
          ...
          <SimpleGrid>
            ...
            >
              {dataFaqsQuery.data
                ? dataFaqsQuery.data.pages.map((page) =>
                    page.data.content.map((item: IPostResponseModel, index: number) => (
                      <PostRender key={`province-faqs-${item.id}-${index}`} post={item} currentUserId={currentUserId} />
                    ))
                  )
                : ArrayTenTemp.map((item, index) => (
                    <>
                      <Box key={`province-faqstmp-${uuidv4()}`} padding='6' boxShadow='lg' bg='white' mb='5' rounded='md'>
              ...
              {dataFaqsQuery.isFetching &&
                ArrayTenTemp.map((item, index) => (
                  <>
                    <Box key={`province-faqsftc-${uuidv4()}`} padding='6' boxShadow='lg' bg='white' mb='5' rounded='md'>
                      <SkeletonCircle size='10' />
                      <SkeletonText my='4' noOfLines={4} spacing='4' />
                      <Skeleton h='xs'></Skeleton>
                    </Box>
